...
/* 若为内核线程,需要重新填充页表为0x100000 */
    ...
    if (p_thread->pgdir != NULL)	{    // 用户态进程有自己的页目录表
        pagedir_phy_addr = vaddr2phyaddr((uint32_t)p_thread->pgdir);
    }
...
/************************** 2  更新页目录地址 **********************************/
    uint32_t new_page_dir_phy_addr = vaddr2phyaddr((uint32_t)page_dir_vaddr);
...
void create_user_vaddr_bitmap(struct task_struct* user_prog) {
    ...
    user_prog->userprog_vaddr.virtual_bitmap.flags = apply_kernel_pages(bitmap_pg_cnt);
    user_prog->userprog_vaddr.virtual_bitmap.len = (0xc0000000 - USER_VADDR_START) / PG_SIZE / 8;
    init_bitmap(&user_prog->userprog_vaddr.virtual_bitmap);
}
